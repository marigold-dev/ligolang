...
let compile_type :
 ...
 fun ~raise (t : AST.type_expression) ->
  t
  |> Aggregation.compile_type ~raise
  |> Spilling.compile_type ~raise
...
and compile_expr ~raise eenv expr =
  ...
  | E_constant const ->
      let match_prim = function
        ...
        | C_AND ->
            Land
        | C_EDIV ->
            failwith "todo" (* encode with prim or ligo code? *)
        | _ ->
          ...
          App
            { funct =
                App
                  ...
                  ; arg =
                      compile_expr ~raise eenv @@ List.hd_exn const.arguments
                  }
  ...
  | E_let_in expr ->
      compile_let_in ~raise eenv expr
  ...
  | E_record_accessor { record; path } ->
      ...
      let label =
        ...
        | T_record rows ->
            let[@warning "-8"] (Some (label, _)) =
              ...
              List.findi bindings ~f:(fun _ (k, _) -> Poly.(k = path))
            in
      ...
      let access =
        match label with
        | 2, 0 ->
            App { funct = Prim Fst; arg = var }
        | 2, 1 ->
            App { funct = Prim Snd; arg = var }
        | _size, _idx ->
            (* FIX ME*)
            var
      in
  ...
  | E_constructor { constructor = Label "TRUE"; element = _ } ->
      Const 1L
  | E_constructor { constructor = Label "FALSE"; element = _ } ->
      Const 0L
  | E_constructor { constructor = Label constructor; element } ->
      let idx =
        ...
        | T_constant const
          when String.(
                 Stage_common.Constant.to_string const.injection = "option") ->
          ...
          | "None" ->
              1
          | x ->
              failwith @@ Format.asprintf "invalid constructor %s" x )
        | x ->
            failwith
            @@ Format.asprintf
                 "Unsupported type in pattern match: %a\n"
                 AST.PP.type_content
                 x
      in
      ( match element.expression_content with
      | T.E_literal Literal_unit ->
          Const (Int.to_int64 idx)
      | _ ->
          Pair
  ...
  | _ ->
      failwith @@ Format.asprintf "unsupported %a" AST.PP.expression expr
...
and compile_pattern_matching ~raise environment ~matchee cases =
  ...
  let pattern_match_common cases fn =
      ...
      List.map
        ~f:(fun { T.constructor = Label lbl; pattern = ppat; body } ->
          let eenv =
            Map.set
              environment
              ~key:(ident_of_var ppat)
              ~data:(compile_expr ~raise environment body)
          in
    ...
    @@ List.drop_last_exn code
  in
  match (compiled_type, cases) with
  | T_or _, { cases; _ } ->
      let rows =
        matchee.type_expression.type_content
        |> function
        | T_sum r | T_record r ->

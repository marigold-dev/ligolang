...
let test_w name f =
  Test (Alcotest.test_case name `Quick @@ fun () -> wrap_test_w name f)
...
let wrap_ref file f =
  ...
  fun () ->
    match !s with
    | Some (a, file') -> if file' = file then a else f s
    | None -> f s
...
let to_zinc ~raise ?(st = "auto") f entry options =
  Ligo_compile.Utils.to_zinc ~raise ~options f st entry
...
let pack_payload ~raise (env : Ast_typed.environment)
    (payload : Ast_imperative.expression) : bytes =
  let code =
    let sugar = Ligo_compile.Of_imperative.compile_expression ~raise payload in
    let core = Ligo_compile.Of_sugar.compile_expression sugar in
    let typed, _ =
      Ligo_compile.Of_core.compile_expression ~raise ~infer:options.infer ~env
        core
    in
    let mini_c = Ligo_compile.Of_typed.compile_expression ~raise typed in
    Ligo_compile.Of_mini_c.compile_expression ~options mini_c
  in
...
let sign_message ~raise (env : Ast_typed.environment)
    (payload : Ast_imperative.expression) sk : string =
...
let typed_program_to_michelson ~raise (program, env) entry_point =
  ...
  let michelson =
    Ligo_compile.Of_mini_c.aggregate_and_compile_contract ~raise ~options mini_c
      entry_point
  in
  let michelson =
...
let typed_program_with_imperative_input_to_michelson ~raise
    ((program, env) : Ast_typed.module_fully_typed * Ast_typed.environment)
    (entry_point : string) (input : Ast_imperative.expression) :
  ...
  let typed_app, _env =
    Ligo_compile.Of_core.compile_expression ~raise ~infer:options.infer ~env app
  in
  let compiled_applied =
    Ligo_compile.Of_typed.compile_expression ~raise typed_app
  in
  let mini_c_prg = Ligo_compile.Of_typed.compile ~raise program in
  Ligo_compile.Of_mini_c.aggregate_and_compile_expression ~raise ~options
    mini_c_prg compiled_applied

let run_typed_program_with_imperative_input ~raise ?options
    ((program, env) : Ast_typed.module_fully_typed * Ast_typed.environment)
    (entry_point : string) (input : Ast_imperative.expression) :
    Ast_core.expression =
  let michelson_program =
    typed_program_with_imperative_input_to_michelson ~raise (program, env)
      entry_point input
  in
  let michelson_output =
    Ligo_run.Of_michelson.run_no_failwith ~raise ?options michelson_program.expr
      michelson_program.expr_ty
  in
  let res =
    Decompile.Of_michelson.decompile_typed_program_entry_function_result ~raise
      program entry_point (Runned_result.Success michelson_output)
  in
  match res with
...
let expect_fail ~raise ?options program entry_point input =
  trace ~raise (test_run_tracer entry_point) @@ fun ~raise ->
  Assert.assert_fail ~raise test_expected_to_fail
  @@ run_typed_program_with_imperative_input ?options program entry_point input

let expect_string_failwith ~raise ?options program entry_point input
    expected_failwith =
  let michelson_program =
    typed_program_with_imperative_input_to_michelson ~raise program entry_point
      input
  in
  let err =
    Ligo_run.Of_michelson.run_failwith ~raise ?options michelson_program.expr
      michelson_program.expr_ty
  in
  match err with
...
let expect_evaluate ~raise (program, _env) entry_point expecter =
  ...
  let mini_c = Ligo_compile.Of_typed.compile ~raise program in
  let exp, _ =
    trace_option ~raise unknown @@ Mini_c.get_entry mini_c entry_point
  in
  let michelson_value =
    Ligo_compile.Of_mini_c.aggregate_and_compile_expression ~raise ~options
      mini_c exp
  in
  let res_michelson =
    Ligo_run.Of_michelson.run_no_failwith ~raise michelson_value.expr
      michelson_value.expr_ty
  in
  let res =
    Decompile.Of_michelson.decompile_typed_program_entry_expression_result
      ~raise program entry_point (Success res_michelson)
  in
  let res' =
    match res with
    | Runned_result.Success exp -> exp
...
let expect_n_aux ~raise ?options lst program entry_point make_input
    make_expecter =
  let aux n =
...
let expect_eq_n_trace_aux ~raise ?options lst program entry_point make_input
    make_expected =
  let aux n =
    let input = make_input n in
...
let expect_eq_exp_trace_aux ~raise ?options explst program entry_point
...
let expect_failwith_exp_trace_aux ~raise ?options explst program entry_point
...
let expect_eq_n_aux ~raise ?options lst program entry_point make_input
    make_expected =
  let aux n =
    let input = make_input n in
    let expected = make_expected n in

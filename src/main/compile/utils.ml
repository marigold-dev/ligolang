...
let to_imperative ~raise ~add_warning ~options ~meta (c_unit : Buffer.t)
    file_path =
  let () = ignore options in
  let imperative =
    Of_c_unit.compile ~raise ~add_warning ~meta c_unit file_path
  in
  imperative

let to_sugar ~raise ~add_warning ~options ~meta (c_unit : Buffer.t) file_path =
  let imperative =
    to_imperative ~raise ~add_warning ~options ~meta c_unit file_path
  in
...
let type_file ~raise ~add_warning ~options f stx form :
    Ast_typed.module_fully_typed * Ast_typed.environment =
...
let compile_file ~raise ~add_warning ~options f stx ep =
  ...
  let mini_c = Of_typed.compile ~raise typed in
  let michelson =
    Of_mini_c.aggregate_and_compile_contract ~raise ~options mini_c ep
  in
...
let type_contract_string ~raise ~add_warning ~options syntax expression env =
  ...
  let core = Of_sugar.compile sugar in
  let inferred =
    Of_core.infer ~raise ~options:{ options with init_env = env } core
  in
  let typed, e =
    Of_core.typecheck ~raise ~add_warning
      ~options:{ options with init_env = env }
      Env inferred
  in
  (typed, core, e)
...
let type_expression ~raise ~options source_file syntax expression env =
  ...
  let c_unit_exp, _ =
    Of_source.compile_string ~raise ~options ~meta expression
  in
  let imperative_exp = Of_c_unit.compile_expression ~raise ~meta c_unit_exp in
  let sugar_exp = Of_imperative.compile_expression ~raise imperative_exp in
  let core_exp = Of_sugar.compile_expression sugar_exp in
...
let expression_to_mini_c ~raise ~options source_file syntax expression env =
  let typed_exp, _ =
    type_expression ~raise ~options source_file syntax expression env
  in
...
let compile_expression ~raise ~options source_file syntax expression env =
  let mini_c_exp =
    expression_to_mini_c ~raise ~options source_file syntax expression env
  in
  let compiled = Of_mini_c.compile_expression ~options mini_c_exp in
  compiled

let compile_and_aggregate_expression ~raise ~options source_file syntax
    expression env mini_c_prg =
  let mini_c_exp =
    expression_to_mini_c ~raise ~options source_file syntax expression env
  in
  let compiled =
    Of_mini_c.aggregate_and_compile_expression ~raise ~options mini_c_prg
      mini_c_exp
  in
  compiled

let compile_storage ~raise ~options storage input source_file syntax env
    mini_c_prg =
  let meta = Of_source.extract_meta ~raise syntax source_file in
  let (storage, _), (input, _) =
    Of_source.compile_contract_input ~raise ~options ~meta storage input
  in
  let imperative =
    Of_c_unit.compile_contract_input ~raise ~meta storage input
  in
  ...
  let compiled =
    Of_mini_c.aggregate_and_compile_expression ~raise ~options mini_c_prg mini_c
  in
  compiled
